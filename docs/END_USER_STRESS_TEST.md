...
| **Post-lock transition** | Automatic after lock expires | One contrast section (B, then later C…) for TRANSITION BARS. Crash + build-up fill on entry. Status `Section: Transition B - CHORUS · bar X/N · Y left`. **Then back to A.** With TRANSITION SECTIONS = 2 the cycle is **A-B-A-C-A**, not A-B-C-A, and it **loops A-B-A-C-A forever** until **Stop**/Forget | Scripted, like Play mode — the bass plays the new section's harmony (not a live mirror of the guitarist). The recorded riff is held through the loop, so replaying it does **not** cut the contrast short and the engine never re-locks onto a NEW riff; change A only by **Stop + Record riff** |
...
|---|---|---|---|
| Groove status | `idle - press Play or Record riff` | ☐ pass ☐ fail | |
| Pattern | 0 | ☐ | |
| Style | Silence (until you play, then it may still not arm) | ☐ | |
| Drums / bass | **Complete silence** even while you chug | ☐ | |
| Scope | Waveform appears when you play; downbeat notch 1 at left; playhead fills left→right | ☐ | |
...
|---|---|---|---|---|
| Count-in | 1 bar of click, no groove yet | ☐ | | |
| Recording | Button `Rec n/4 - N` with N climbing | ☐ | | |
| Lock | `Groove: LOCKED - bar 1/4 - 3 left…` | ☐ | | |
| Drums during lock | **Frozen** — same groove all 4 bars, does not chase your dynamics | ☐ | | |
...
- Record riff: 1-bar count-in, 4-bar capture, lock for lockBars. Bass is note-for-note frozen. Drums frozen. After lock: contrast B for transitionBars, **then back to A**, then (if TRANSITION SECTIONS ≥ 2) contrast C, **then back to A**. Shape is **A-B-A-C-A**, not A-B-C-A, and it **loops A-B-A-C-A forever** until **Stop**/Forget. The recorded riff is held through the loop, so replaying the riff does **not** cut a contrast short and the engine never re-locks onto a NEW riff — change A only by **Stop + Record riff**. Crash in; the bass is scripted (section harmony) during B/C rather than a live mirror.
